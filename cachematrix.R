...
makeCacheMatrix <- function(x = matrix()) {
  ...
  set <- function(y) {
    ...
    j <<- NULL ## re-initialize j in the parent environment to null
  }
  get <- function() x ## return the matrix x
  setinverse <- function(inverse) j <<- inverse ## set the cache j equal to the inverse of the matrix x
  getinverse <- function() j ## return the cached inverse of x
